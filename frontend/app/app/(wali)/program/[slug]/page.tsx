...
export default function ProgramDetailPage({ params }: ProgramDetailPageProps) {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto w-full flex-grow flex flex-col gap-6 pb-12 font-dm text-app-text-mid">
      {/* Back Header */}
      <header className="flex items-center gap-3">
        <button
          onClick={() => router.back()}
          className="w-10 h-10 rounded-[6px] bg-app-surface border border-app-border flex items-center justify-center text-app-text hover:bg-app-white active:scale-95 shadow-sm transition-all"
        >
          <ArrowLeft size={18} />
        </button>
        <div>
      ...
      {!isEnrolled ? (
        <>
          {/* Program info card */}
          <div className="p-6 sm:p-8 flex flex-col gap-4 rounded-2xl border border-app-border bg-app-white shadow-sm">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="inline-block px-2.5 py-0.5 bg-app-primary/10 border border-app-primary/20 text-app-primary rounded-[4px] text-[10px] font-bold uppercase">
                {CATEGORY_LABELS[program.category] || program.category}
              </span>
              {!program.active && (
                <span className="px-2.5 py-0.5 bg-amber-50 text-amber-800 border border-amber-100 rounded-[4px] text-[10px] font-bold">
                  Coming Soon
                </span>
              )}
            </div>
            <h2 className="text-xl sm:text-2xl font-normal text-app-text font-playfair">{program.name}</h2>
            <p className="text-sm text-app-text-mid leading-relaxed">{program.description}</p>

            <div className="grid grid-cols-2 gap-3 border-t border-app-border/40 pt-4 mt-1">
              {program.basePrice ? (
                <div className="bg-app-white border border-app-border rounded-[6px] p-3">
                  <div className="text-[10px] font-bold uppercase tracking-wide text-app-text-muted">
                    Mulai dari
                  </div>
                  <div className="mt-0.5 text-base font-bold text-app-text">
                    Rp {program.basePrice.toLocaleString("id-ID")}
                    <span className="text-xs font-semibold text-app-text-muted">/sesi</span>
                  </div>
                </div>
              ) : null}
              <div className="bg-app-white border border-app-border rounded-[6px] p-3">
                ...
                <div className="mt-0.5 text-base font-bold text-app-text">
                  ...
                  <span className="text-xs font-semibold text-app-text-muted"> / blok</span>
                </div>
              </div>
              <div className="bg-app-white border border-app-border rounded-[6px] p-3">
                <div className="text-[10px] font-bold uppercase tracking-wide text-app-text-muted">
                ...
                <div className="mt-0.5 text-sm font-bold text-app-text truncate">
            ...
            <section className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                {sortedSteps.map((step) => (
                  <div
                    key={step.id}
                    className="p-4 flex items-center gap-3 cursor-pointer rounded-2xl border border-app-border bg-app-white shadow-sm hover:bg-app-surface active:scale-[0.99] transition-all"
                    onClick={() => setActiveModalStep(step)}
                  >
                    <span className="shrink-0 w-8 h-8 rounded-full bg-app-white text-app-text border border-app-border flex items-center justify-center text-xs font-bold">
                      {step.order}
                    </span>
                    <div className="min-w-0 flex-1">
                      <h4 className="text-sm font-bold text-app-text">{step.title}</h4>
                      <p className="text-xs text-app-text-muted truncate">
                        {step.bodyText.split("\n")[1] || step.bodyText}
                      </p>
                    </div>
                    {step.level ? (
                      <span className="shrink-0 px-2 py-0.5 rounded-[4px] text-[10px] font-bold bg-app-primary/10 border border-app-primary/20 text-app-primary">
                        {step.level}
                      </span>
                    ) : null}
                    <ChevronRight size={16} className="shrink-0 text-app-text-muted" />
                  </div>
                ))}
              </div>
            </section>
          ...
          <div className="mt-2 border border-app-border bg-app-white shadow-sm rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="text-center sm:text-left">
              <h4 className="text-base font-bold text-app-text">
                Tertarik mendaftarkan {selectedMurid?.name || "anak Anda"}?
              </h4>
              <p className="text-xs sm:text-sm text-app-text-muted mt-1">
                {program.basePrice ? (
                  <>
                    Mulai dari{" "}
            ...
            <Button
              ...
              className="w-full sm:w-auto justify-center gap-1.5 px-4 py-2.5 text-xs sm:text-sm bg-[#25D366] hover:bg-[#1fb959] text-white border-none shadow-md shadow-[#25D366]/25 font-semibold rounded-[6px]"
            >
        ...
        <>
          ...
          {program.hasRoadmap ? (
            <>
              <div className="p-6 sm:p-8 flex flex-col gap-4 rounded-2xl border border-app-border bg-app-white shadow-sm">
                <div>
                  <span className="inline-block px-2.5 py-0.5 bg-app-primary/10 border border-app-primary/20 text-app-primary rounded-[4px] text-[10px] font-bold uppercase mb-2">
                    {program.category}
                  </span>
                  <h2 className="text-lg sm:text-xl font-normal text-app-text font-playfair">{program.name}</h2>
                  <p className="text-xs sm:text-sm text-app-text-muted mt-1">
                ...
                <div className="border-t border-app-border/40 pt-4 mt-2">
                  <div className="flex justify-between items-center text-xs sm:text-sm font-semibold text-app-text-muted mb-1.5">
                    <span>
                    ...
                    <span className="text-app-primary font-bold">{stats.progressPercent}%</span>
                  </div>
                  <div className="h-2.5 w-full bg-app-white border border-app-border rounded-full overflow-hidden shadow-inner">
                    <div
              ...
              {sortedSteps.length > 0 ? (
                <section className="relative mt-2">
                  <div className="absolute left-6 top-8 bottom-8 w-0.5 bg-app-border/30 -z-10"></div>
                  <div
                    className="absolute left-6 top-8 w-0.5 bg-app-primary -z-10 transition-all duration-500"
                      ...
                      return (
                        <div key={step.id} className="flex gap-4 relative">
                          <div className="flex-shrink-0 w-12 flex justify-center pt-2">
                            ...
                            {isActive && (
                              <div className="w-8 h-8 rounded-full bg-app-primary text-white flex items-center justify-center shadow-lg relative z-10 animate-pulse">
                                <Play size={14} fill="currentColor" className="ml-0.5" />
                              </div>
                            )}
                            {isLocked && (
                              <div className="w-8 h-8 rounded-full bg-app-white text-app-text-muted border border-app-border flex items-center justify-center shadow-sm">
                                <Lock size={14} />
                              </div>
                            )}
                          </div>
                          ...
                          <div
                            className={`flex-1 p-5 transition-all duration-300 rounded-2xl border border-app-border bg-app-white shadow-sm ${
                              isActive ? "border-2 border-app-primary shadow-md bg-app-white" : "opacity-80"
                            } ${!isLocked ? "cursor-pointer hover:bg-app-surface" : ""}`}
                            onClick={() => toggleStep(step.id, isLocked)}
                          >
                            <div className="flex justify-between items-start gap-2 mb-2">
                              <h3 className="font-bold text-app-text text-sm sm:text-base text-left">
                                ...
                                <span
                                  className={`px-2 py-0.5 rounded-[4px] text-[10px] font-bold ${
                                    isActive
                                      ...
                                      : "bg-app-white border border-app-border text-app-text-muted"
                                  }`}
                                >
                                  {step.level || "Dasar"}
                                </span>
                            ...
                            <p className="text-xs sm:text-sm text-app-text-muted mb-1 leading-relaxed text-left">
                              {step.bodyText.split("\n")[1] || step.bodyText}
                            </p>

                            {isOpen && (
                              <div
                                className="mt-4 pt-4 border-t border-app-border/40 flex flex-col gap-4"
                              ...
                              >
                                ...
                                <div className="mt-2 flex justify-start">
                                  <Button
                                    ...
                                    className="text-xs py-2 px-3 bg-[#25D366] hover:bg-[#1fb858] text-white border-none shadow-md flex items-center gap-1.5 rounded-[6px] font-bold"
                                  >
                            ...
                            {isLocked && (
                              <div className="inline-flex items-center gap-1.5 text-xs text-app-text-muted font-semibold cursor-not-allowed mt-2">
                                <Lock size={12} />
          ...
          <div className="mt-2 p-5 border border-app-border bg-app-white rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-3">
            <div className="text-xs sm:text-sm font-semibold text-app-text-muted flex items-center gap-1.5">
              <CalendarDays size={16} className="text-app-primary" />
              <span>
                Sesi Selesai:{" "}
                <strong className="text-app-text">
            ...
            <Link href="/app/jadwal" className="w-full sm:w-auto">
              <Button
                variant="ghost"
                className="w-full text-xs justify-center py-2.5 border border-app-primary/30 text-app-primary bg-app-white hover:bg-app-surface rounded-[6px]"
              >
      ...
      {activeModalStep && (
        ...
        >
          <div
            className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl border border-app-border bg-app-white p-6 shadow-md flex flex-col gap-4"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                type="button"
                onClick={() => setActiveModalStep(null)}
                className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-[6px] bg-app-white border border-app-border text-app-text-muted shadow-sm hover:bg-app-surface active:scale-90 transition-all"
