...
| Table | Fields | Notes |
|---|---|---|
| **`users`** | id, role, name, phone, email, created_at | role: `admin` \| `tentor` \| `wali` |
...
| **`sessions`** | id, program_id, tentor_id, murid_id, starts_at, ends_at, location, status | status: `scheduled` \| `cancelled` \| `completed` |
...
| **`daily_reports`** | id, session_id, murid_id, date, start_time, end_time, activity, notes | Diisi tentor per sesi |
| **`progress_reports`**| id, murid_id, program_id, block_number, achievements, mastered_materials, weak_materials, notes, created_at | Diisi tentor per blok (N sesi) |
...
- `users` (1) ↔ `sessions` (1:N) (Penugasan Tentor ke Sesi)
...
- **`notes`**: Text (Catatan khusus tentor mengenai murid pada sesi tersebut).
...
- **`notes`**: Text (Saran/catatan dari tentor untuk wali murid).
