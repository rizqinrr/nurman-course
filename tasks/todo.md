...
- [ ] NC-1.6 Test harness: Vitest + supertest, Prisma di-mock (fokus guard & validasi, bukan query) — (in_progress, 2026-09-16); implementasi/verifikasi selesai, final commit/push masih diproses. Dikerjakan sebelum NC-1.4, tanpa mengubah perilaku auth atau staging.
  - [x] 2026-09-16 NC-1.6a Pin Vitest 4.1.11, supertest 7.1.4, @types/supertest 6.0.3; 3.2.4 awal diganti karena advisory. Review lockfile: PostCSS/Nano ID/hasown/tinyglobby ikut naik karena kebutuhan transitive, @types/estree patch incidental. Tidak menjalankan audit fix global; dependency frontend shared ikut terpengaruh dan build diverifikasi.
  - [x] 2026-09-16 NC-1.6b Config Node-only, discovery `tests/**/*.test.ts`, tsconfig test terpisah dari production CJS. Script `test` aggregate node:test + Vitest; `test:clients`, `test:api`, `test:watch`, `typecheck:test` tersedia. Empty discovery exit1.
  - [x] 2026-09-16 NC-1.6c Setup env palsu, dotenv/Prisma/SDK mock, reset tiap test, unexpected-call ledger; TCP hanya ke loopback/port suite, fetch/TLS/listener lain diblok. Enam probe sementara membuktikan pelanggaran yang ditelan tetap gagal teardown; aggregate exit1. Probe dihapus setelah verifikasi, tidak masuk commit.
  - [x] 2026-09-16 NC-1.6d 18 auth characterization pass: missing/malformed header, SDK error/null, admin/tentor/wali, SDK throw500, legacy fallbackwali, nonadmin/unknown/absent403, adminpass, isolasi identitas. Middleware tidak di-mock, role DB belum diterapkan.
  ...
  - [ ] NC-1.6g Finalisasi docs/staged review, commit dan push NC-1.6 ke `be-restruktur`, verifikasi remote sama HEAD. NC-1.4 tidak otomatis dimulai.
...
- NC-1.6 — implementasi/verifikasi selesai, final commit/push `be-restruktur` sedang diproses. NC-1.4 belum dimulai. **WALI-MOB di-pause** (per keputusan user 2026-09-15: fokus pindah ke backend).
