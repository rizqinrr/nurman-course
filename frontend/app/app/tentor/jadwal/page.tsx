...
export default function TentorJadwalPage() {
  const [selectedMuridId, setSelectedMuridId] = useState("all");
  ...
  const upcomingSessions = filteredSessions.filter(s => s.status === "scheduled").sort((a,b) => a.startsAt.localeCompare(b.startsAt));
  const pastSessions = filteredSessions.filter(s => s.status === "completed" || s.status === "cancelled").sort((a,b) => b.startsAt.localeCompare(a.startsAt));

  const hasReport = (sessionId: string) => dailyReports.some(r => r.sessionId === sessionId);
  ...
  const openEdit = (s: SessionWithRelations) => {
    const raw = sessionsDataRaw.find((r: SessionWithRelations) => r.id === s.id);
    if (!raw) return;
  ...
  if (loading) {
    return (
      <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto w-full flex-grow flex flex-col justify-center items-center gap-4">
        ...
        <div className="text-gray-500 font-semibold animate-pulse">Memuat jadwal mengajar...</div>
      </div>
    );
  }
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto w-full flex-grow flex flex-col gap-5 sm:gap-6 pb-32 animate-[fadeIn_0.5s_ease-out]">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-[20px] bg-white/45 backdrop-blur-xl border border-white/60 shadow-sm p-4 sm:p-5">
        <div>
          ...
          <p className="text-[12px] sm:text-sm text-gray-600 leading-relaxed mt-0.5">Buat jadwal manual per murid • Muncul di dashboard & wali murid</p>
        </div>
      ...
      {enrollments.length===0 && (
        <GlassCard className="p-4 border border-amber-200 bg-amber-50/60">
          <p className="text-sm text-amber-800"><AlertTriangle size={14} className="inline mr-1" /> Belum ada enrollment aktif. Minta admin hubungkan kamu dengan murid di <span className="font-bold">/app/admin/enrollment</span> dulu.</p>
        </GlassCard>
      )}

      <div className="flex flex-col gap-1.5 bg-white/20 backdrop-blur-md p-4 rounded-2xl border border-white/40 shadow-sm">
        <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wider pl-1">Filter Siswa</span>
        <div className="flex flex-wrap gap-1.5">
          <button onClick={() => setSelectedMuridId("all")} className={`px-3.5 py-1.5 rounded-full text-xs font-semibold border transition-colors ${selectedMuridId==="all" ? "bg-[#4a70a9] text-white border-transparent shadow-sm" : "bg-white/40 text-gray-600 hover:bg-white/60 border-white/60"}`}>Semua Siswa</button>
          {muridsList.map((m) => (
            <button key={m.id} onClick={() => setSelectedMuridId(m.id)} className={`px-3.5 py-1.5 rounded-full text-xs font-semibold border transition-colors ${selectedMuridId===m.id ? "bg-[#4a70a9] text-white border-transparent" : "bg-white/40 text-gray-600 border-white/60"}`}>{m.name}</button>
          ))}
        </div>
      </div>
            ...
            {!editingSession && (
              <label className="block text-xs font-bold text-gray-700">
                ...
                <select required value={form.enrollmentId} onChange={(e) => {
                  ...
                  {enrollments.map((enr) => (
                    <option key={enr.id} value={enr.id}>{enr.murid?.name || enr.muridId} • {enr.program?.name || enr.programId}</option>
                  ))}
                </select>
              </label>
            )}
        ...
        {upcomingSessions.length > 0 ? (
          ...
          <GlassCard className="p-6 border border-white/60 shadow-sm flex flex-col items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1 max-w-sm">Buat jadwal baru lewat tombol Buat Jadwal. Pilih murid yang terhubung dengan enrollment kamu.</p>
            </div>
                ...
                <GlassCard key={session.id} className="p-4 border border-white/80 shadow-sm flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                  <div className="flex-grow">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500">
                      <span className="flex items-center gap-1"><User size={13} className="text-[#4a70a9]" /><span className="font-semibold text-gray-700">{murid.name}</span></span>
                      <span className="flex items-center gap-1"><BookOpen size={13} className="text-[#4a70a9]" />{program?.name}</span>
                      <span className="flex items-center gap-1"><Clock size={13} className="text-[#4a70a9]" />{session.startsAt.split(" • ")[1] || "WIB"}</span>
                    </div>
                  </div>
                  <div className="shrink-0 w-full md:w-auto self-stretch md:self-center flex items-center">
                    {!isCancelled && (
                      reported ? (
                        <Link href={`/app/tentor/laporan-harian?editSessionId=${session.id}`} className="w-full"><Button variant="ghost" className="w-full text-xs justify-center py-2 px-4 border border-[#4a70a9]/20 text-[#4a70a9]">Lihat Laporan</Button></Link>
                      ) : (
                        <Link href={`/app/tentor/laporan-harian?sessionId=${session.id}`} className="w-full"><Button variant="primary" className="w-full text-xs justify-center py-2 px-4">Tulis Laporan</Button></Link>
                      )
                    )}
                  </div>
                </GlassCard>
