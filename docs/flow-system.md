...
Foto/bukti menggunakan string/data URL base64; invoice image diproses client, PDF/prabayar dibaca melalui FileReader. Ini bukan upload ke public Storage dan base64 bukan enkripsi. Private bucket/signed URL belum menjadi alur selesai. UI menampilkan batas file, tetapi backend masih memakai `express.json()` tanpa limit khusus: label UI bukan jaminan payload diterima server.
